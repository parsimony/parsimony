 ...
 *  @version  Release: 1.0
 *  @category  HTML5editor - jQuery Plugin
...
function wysiwyg() {
    ...
    this.buildToolbar = function(){
        ...
        toolbar.html('<iframe id="popover"></iframe><ul class="tabs"></ul><div class="commands"></div>');
...
function wysiwyg_selectSquel() {
	...
	var prop = $('#popover').css({width:"150px",height:"200px"}).appendTo($(e.target).closest(".btn")).show().contents().find("body").empty().append(html);
        var $this = this;
...
function wysiwyg_colorPicker() {
	...
	var prop = $('#popover').css({width:"200px",height:"165px"}).appendTo($(e.target).closest(".btn")).show().contents().find("body").empty().append(html);
...
function wysiwyg_insertImage() {
	...
	var prop = $('#popover').css({width:"300px",height:"50px"}).appendTo(e.target.parentNode).show().contents().find("body").empty().append(html);
        var $this = this;
	$(".addIMG",prop).on("click",function(e){
	    editor.setCommand($this.command,$(".URLimg",prop).val());
	    $('#popover').hide();
	    $(".addIMG",prop).off("click");
	});
    }

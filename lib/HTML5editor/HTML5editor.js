...
function wysiwyg() {
	...
	var el = this.currentDocument.querySelector(this.selector);
	  
	/* If it's not already done */
	
	if(this.toolbarDocument.querySelectorAll('.HTML5editorToolbar[data-selector="' + this.selector + '"]').length == 0){
		...
		this.textarea = el;
		this.textarea.parentNode.insertBefore(this.toolbar);
		var singleEditor = this.toolbarDocument.createElement("div");
		singleEditor.id = "editor" + (this.currentDocument.querySelectorAll(".HTML5editorToolbar").length + 1);
		singleEditor.setAttribute("contenteditable","true");
		singleEditor.setAttribute("spellcheck", "false");
		singleEditor.setAttribute("data-textarea",this.selector);
		singleEditor.style = el.style;
		singleEditor.classList.add("HTML5editorEditor");
		singleEditor.innerHTML = this.textarea.value;
		this.currentElmt = singleEditor;
		this.textarea.style.display = "none";
		this.textarea.parentNode.insertBefore(singleEditor);
		this.selector = "#" + singleEditor.id;
	    }else{
		/* If multiple mode: one toolbar for severals contenteditable divs */
		this.toolbarDocument.querySelector("body").appendChild(this.toolbar);
		this.toolbar.classList.add("multiple");
	...
	[].forEach.call( this.currentDocument.querySelectorAll(this.selector), function(el) {
		...
		el.addEventListener('focus', $this.prepareDIV.bind($this), false);
	    }else{
		/* Copy the cotent of WYSIWYG to the textarea */
		el.addEventListener('keyup', function(e) {
		    $this.textarea.value = el.innerHTML;
		    e.preventDefault();
		}, false);
		el.addEventListener('blur', function(e) {
		    $this.textarea.value = $this.currentElmt.innerHTML;
		}, false);
	    }
	});
	...
	if(this.isMultiple) $this.setDIV(this.currentDocument.querySelector(this.selector).id);	
	...
	if(this.enable){
	    this.setDIV(e.target.id);
	    e.preventDefault();
	}
    }
    
    this.toolbarAction = function (e) {
	e.stopImmediatePropagation();
	e.preventDefault();
	this.widgets[e.target.getAttribute("data-name")].onClick(e, this);
    }
    
    this.onClickToolbarTab = function (e) {
	...
	this.toolbar.querySelector(e.target.getAttribute("href").replace("#",".")).style.display = "block";
	e.preventDefault();
    }
    
    this.buildToolbar = function(){
	var toolbar = this.toolbarDocument.createElement("div");
	toolbar.classList.add("HTML5editorToolbar");
	toolbar.setAttribute("data-selector",this.selector);
	...
	for (var i=0; i < this.toolbarWidgets.length; i++) {
	    ...
	    if(!toolbar.querySelector(".toolbar_" + widget.category)){
		...
		toolbar.querySelector(".tabs").appendChild(item);
		var tab = this.toolbarDocument.createElement("div");
		tab.classList.add("toolbar_" + widget.category);
		toolbar.querySelector(".commands").appendChild(tab);
	    }
	    toolbar.querySelector(".toolbar_" + widget.category).appendChild(tag); 	
	}

	$this = this;
	
	/* Manage Tabs */
	[].forEach.call( toolbar.querySelectorAll(".tabs a"), function(el) {
	    el.classList.add("current");
	    el.addEventListener('click', $this.onClickToolbarTab.bind($this), false);
	});
	
	/* Listen click action on BTN && Listen change action on selects*/
	[].forEach.call( toolbar.querySelectorAll(".HTML5editorAction"), function(el) {
	    el.addEventListener('click', $this.toolbarAction.bind($this), false);
	    el.addEventListener('change', $this.toolbarAction.bind($this), false);
	});
	...
	if(this.isMultiple) this.toolbar.style.width = this.currentElmt.offsetWidth + "px";
	var offset = $(this.currentElmt).offset();
	var top = offset.top - 55;
	if(top < 0){
	    top = offset.top + this.currentElmt.innerHeight;
	}
	this.toolbar.style.top = top + "px";
	this.toolbar.style.left = offset.left + "px";
    }
...
function wysiwyg_createLink() {
	...
	if(link && link.length > 0){
	    top.setCommand("createLink", link);
	}
    }
}
...
function wysiwyg_formatBlock() {
	...
	[].forEach.call( select.querySelectorAll(".option"), function(el) {
	    el.addEventListener('click', function(e) {
		top.setCommand($this.command,this.innerText);
...
function wysiwyg_colorPicker() {
	...
	c.addEventListener('click', function(event){
	    ...
	    top.setCommand($this.command,myPicker.querySelector('.picked_color_rgb').innerHTML);

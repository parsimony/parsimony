function wysiwyg() {
    ...
    this.init = function (selector, toolbarWidgets, toolbarDocument, currentDocument) {
        ...
        $(this.currentDocument).on("click",this.selector,function(e){
            $this.checkCommands();
            e.preventDefault();
        });
        
        $(this.currentDocument).on("paste drop",this.selector,function(e){
            $this.sanitize(this);
        });
    ...
    this.buildToolbar = function(){
        ...
        toolbar.html('<ul class="tabs"></ul><div class="commands"></div>');
    ...
    this.setDIV = function (currentElmt) {
        ...
        if(top < 0){
            top = offset.top + $(this.currentElmt).height() + 28;
        }
...
function wysiwyg_formatBlock() {
    ...
    this.category = "format";
    this.icon = "data:image/png;base64,";
    ...
    this.getAdmin = function(){
        var html = '<div class="HTML5editorAction formatBlock" data-name="' + this.name + '" data-command="' + this.command + '"><span class="value" style="display:block;width:25px;height:25px;background:url(' + this.icon + ') no-repeat center;"></span><div style="position:absolute;box-shadow: 1px 1px 4px #888;border: 1px solid #BBB;display:none;padding:5px;background:#f9f9f9;" class="propositions">';
    ...
    this.onClick = function(e, editor){
        $('.propositions',e.target).show();
        var $this = this;
	$(".formatBlock .option").on("click",function(e){
	    editor.setCommand($this.command,this.innerText);
	    $('.formatBlock .propositions').hide();
	    e.stopPropagation();
	    $(".formatBlock .option").off("click");
	});
    }
...
function wysiwyg_colorPicker() {
    ...
    this.getAdmin = function(){
        ...
        var html = '<div class="HTML5editorAction" style="position:relative;width:25px;height:25px;background:url(' + this.icon + ') no-repeat center;" data-name="' + this.name + '"data-command="' + this.command + '">';
    ...
    this.onClick = function(e, editor){
        var myPicker = $('.foreColorPicker',e.target);
        myPicker.show();
        var c = $('.mycolorpicker',e.target).get(0);
	...
	$(c).on("mousemove", function(event){
            ...
            $('.picked_color_preview',myPicker).css("background-color",rgb);
            $('.picked_color_rgb',myPicker).text(rgb);
        });
	$(c).on("mousedown", function(event){
            event.stopImmediatePropagation();
            editor.setCommand($this.command,$('.picked_color_rgb',myPicker).text());
            myPicker.hide();
	    $(c).off("mousemove");
	    $(c).off("mousedown");
        });

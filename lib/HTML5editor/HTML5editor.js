function wysiwyg() {
	...
	this.init = function(selector, toolbarWidgets, toolbarDocument, currentDocument) {
		...
		if ($('.HTML5editorToolbar[data-selector="' + this.selector + '"]', this.toolbarDocument).length == 0) {
			...
			$(this.toolbar).on("mousedown.HTML5editor", "*", function(e) {
				e.preventDefault();
				return setTimeout('', 300);
			})
			/* Listen click action on BTN && Listen change action on selects*/
			.on("click.HTML5editor", ".HTML5editorAction", function(e) {
		...
		$(this.currentDocument).on("mouseup.HTML5editor", this.selector, function(e) {
			$this.checkCommands();
			document.querySelector(".popover").style.display = "none";
		})
		.on("paste.HTML5editor drop.HTML5editor", this.selector, function(e) {
			var eltm = this;
			setTimeout(function() {
				$this.sanitize(eltm);
			}, 20);
		})
		.on("keydown.HTML5editor", this.selector, function(e) {
		...
		$(this.selector, this.currentDocument).each(function() {
			if ($this.isMultiple) {
				...
				$(this).on("focus.HTML5editor", function(e) {
					$this.setDIV(e.target);
				})
				.on("input.HTML5editor", function(e) {
					$this.checkCommands();
				});

			} else {
				/* Copy the content of WYSIWYG to the textarea */
				$(this).on("input.HTML5editor", function(e) {
					$this.textarea[0].value = $this.currentElmt.innerHTML;
					$this.checkCommands();
					e.preventDefault();
				})
				.on("blur.HTML5editor", function(e) {
					this.innerHTML = $this.format(this.innerHTML);
				});

				/* Copy the content of textarea to the WYSIWYG */
				$this.textarea.on("keyup.HTML5editor blur.HTML5editor", function(e) {
	...
	this.setDIV = function(currentElmt) {
		...
		var height = this.toolbar.getBoundingClientRect().height;
		var top = dim.top - height + window.pageYOffset - 2;
		if (top < 0) {
			top = dim.top + dim.height + height;
		}
	...
	this.destroy = function() {
		if (typeof this.toolbar != "undefined") {
			...
			$(this.toolbar).off(".HTML5editor", "**");
			$(this.currentDocument).off(".HTML5editor", "**");
			$(this.selector, this.currentDocument).attr("contenteditable", "false").off(".HTML5editor", "**");
			$(this.toolbar).remove();
		}
	...
	this.widgets = {
		btn : function () {
			...
			this.getAdmin = function() {
				return '<div style="background:url(' + BASE_PATH + 'lib/HTML5editor/sprites777.png) no-repeat ' + this.position + ';" class="HTML5editorAction btnWYSIWYG" data-name="' + this.name + '" data-command="' + this.command + '"></div>';
			}
		...
		colorPicker:function () {
			...
			this.getAdmin = function() {
				return '<div class="HTML5editorAction btnWYSIWYG" style="background:url(' + BASE_PATH + 'lib/HTML5editor/sprites777.png) no-repeat ' + this.position + ';" data-name="' + this.name + '"data-command="' + this.command + '"><div class="temoin" style="position:absolute;top:22px;height:3px;left:2px;right:3px;"></div></div>';
			}
		...
		insertImage:function () {
			...
			this.onClick = function(e, editor) {
				...
				if(typeof top.ParsimonyAdmin != "undefined") html += '<div class="tabBtn" id="addImgExplorer">From Explorer</div></div>';
				html += '<div id="url" class="panel"><input type="text" placeholder="http://..." id="URLimg"><input type="button" value="Add" id="addImgURL"></div>';
				...
				$("#addImgExplorer", prop).on("mousedown", function(e) {
					e.preventDefault();
					top.window.callbackExplorer = function (file){
						editor.setCommand($this.command, file);
						top.window.callbackExplorer = function(file){return false;};

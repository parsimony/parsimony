...
function wysiwyg_formatBlock() {
    ...
    this.getAdmin = function(){
	var html = '<div class="HTML5editorAction" data-name="' + this.name + '" data-command="' + this.command + '"><span class="value">H1</span><div style="position:absolute;display:none;padding:5px;background:#ddd;" class="propositions">';
	for (var i = 0; i < this.values.length; i++) {
	    html += '<div class="option">' + this.values[i] + '</div>';
	}
	html += "</div></div>"
	return html;
    }
	...
	var $this = this;
	[].forEach.call( select.querySelectorAll(".option"), function(el) {el.addEventListener('click', function(e) {
		HTML5editor.setCommand($this.command,this.innerHTML);
...
function wysiwyg_colorPicker() {
	...
	html += '<div style="position:absolute;display:none;padding:5px;background:#ddd;" class="foreColorPicker">';
	html += '<div style="float:left;width:22px;height:22px" class="picked_color_preview"></div>';
	html += '<div style="float:left;width:50px;height:22px" class="picked_color_rgb"></div><div style="clear:both"></div>';
	...
	c.addEventListener('mousemove', function(event){
	    ...
	    var rgb = 'rgb(' + img_data[0] + ',' + img_data[1] + ',' + img_data[2] + ')';

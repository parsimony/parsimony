function wysiwyg() {
    ...
    this.buildToolbar = function(){
        ...
        $(toolbar).on("mousedown change",".HTML5editorAction",function(e){
            ...
            e.preventDefault();
            $this.widgets[$(e.target).attr("data-name")].onClick(e, $this);
        });
    ...
    this.setDIV = function (currentElmt) {
        ...
        if(top < 0){
            top = offset.top + $(this.currentElmt).height();
        }
        ...
        for(t in tab){
            ...
            if(typeof tag[1] == "string" && typeof tag[2] == "string"){
                ...
                for(var i=1; i<attrs.length -1;i = i + 3){
                    var name = attrs[i];
                    var value = attrs[i + 1];
                    if(this.inArray(this.allowedTags[currentTag],name)){
                        var newValue = "";
                        if(name == "style"){
    ...
    this.inArray = function(array, p_val) {
        var l = array.length;
        for(var i = 0; i < l; i++) {
            if(array[i] == p_val) {
                return true;

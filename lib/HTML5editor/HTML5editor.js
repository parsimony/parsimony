/*(function( $ ){
    ...
    $.fn.parsiWYSIWYG = function( method ) {
	if ( methods[method] ) {
...
function wysiwyg() {
    ...
    this.checkCommands = function () {
        ...
        $("[data-command]",this.toolbar).each(function(){
            var el = $(this).parent();
            if($this.currentDocument.queryCommandEnabled($(this).attr("data-command"))){
                el.removeClass("inactive"); 
            }else{
            /*if(!el.classList.contains("inactive")) el.classList.add("inactive");*/
            }
            if($this.currentDocument.queryCommandState($(this).attr("data-command"))){

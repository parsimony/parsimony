...
function wysiwyg() {
	...
	if(!this.toolbarDocument.getElementById("HTML5editorToolbar")){
		...
		el.addEventListener('click', function(e) {
		    $this.widgets[this.getAttribute("data-command")].onClick(e);
		    e.preventDefault();
		}, false);
	    });
...
function wysiwyg_btn() {
	...
	return '<button style="background:url(' + this.icon + ');width:22px;height:22px;border:0;" class="HTML5editorAction" data-command="' + this.command + '"></button>';
    }

function wysiwyg() {
	...
	.html('<iframe src="javascript:void(0)" class="popover"></iframe><ul class="tabs"></ul><div class="commands"></div>');
	...
	/* Manage Tabs */
        $(toolbar).on("click",".tabs a",function(e){
	...
	.on("click change",".HTML5editorAction",function(e){
...
function wysiwyg_colorPicker() {
    ...
    this.onClick = function(e, editor){        alert( 5);
	...
	html += '<canvas style="width:180px;height:180px;" class="mycolorpicker"></canvas></div>';
        alert( $('.popover',editor.toolbar));
	var prop = $('.popover',editor.toolbar).css({
	    width:"200px",
	    height:"185px"
	}).appendTo($(e.target).closest(".btn")).show().contents().find("body").append(html);
	...
	$(".pick",prop).on("click.Wcolorpicker", function(event){

...
<style type="text/css">
    ...
    #field_list .myfield{position:relative;font-size: 15px;color: #222;width: 194px;margin: 2px;cursor: move;text-align: left;padding:5px;background-color: #F1F5F9;background-repeat:no-repeat;padding-left:20px;background-position: 2px 5px ;border: 1px solid #97B2D2;}
...
<div class="visibilityform">
	...
	$tab = array('name' => $className, 'oldName' => $className, 'title' => $entity->behaviorTitle, 'description' => $entity->behaviorDescription, 'keywords' => $entity->behaviorKeywords, 'image' => $entity->behaviorImage);
    ...
    <div id="rightsidebar" style="z-index:999">
        <div id="update_table">
            ...
            <div><h3><?php echo t('Fields Behaviour', FALSE); ?></h3>
                ...
                <div class="rightbar"><label class="ellipsis"><?php echo t('Description', FALSE); ?> </label><input type="text" name="description"></div>
                <div class="rightbar"><label class="ellipsis"><?php echo t('Keywords', FALSE); ?></label><input type="text" name="keywords"></div>
                <div class="rightbar"><label class="ellipsis"><?php echo t('Image', FALSE); ?></label><input type="text" name="image"></div>
    ...
    <script>
        ...
        var dbadmin = {
            ...
            createTable : function(tablename){
                if(tablename.length>0){
                    var keywordsReserveds = ',include,require,include_once,require_once,for,foreach,as,if,elseif,else,while,do,endwhile,endif,switch,case,endswitch,endfor,endforeach,return,break,continue,self,static,parent,';
                    if(keywordsReserveds.indexOf("," + tablename + ",") == -1){
                        if(!$('#table_' + tablename).length){
                            $("#container_bdd").prepend('<div id="table_' + tablename + '"data-attributs=\'{"name":"' + tablename + '","oldName":"' + tablename + '","title":"","description":"","keywords":"","image":""}\' class="table" style="left:300px;top:50px;"><div class="title">' + tablename + '</div><div type_class="field_ident">'+ t('ID') +'</div></div>');
		...
		/* Save all models */
			...
			var tableAttrs = $(this).data("attributs");
                        propertylist += '{"name": "' + recupId + '","oldName": "' + tableAttrs.oldName + '","title":"' + tableAttrs.title + '","description":"' + tableAttrs.description + '","keywords":"' + tableAttrs.keywords + '","image":"' + tableAttrs.image + '","top": "'+ $(this).css("top")+'","left": "'+ $(this).css("left")+'","properties" : {';

...
<style>
    ...
    .explorer_file_name{position:absolute;bottom:2px;text-overflow: ellipsis;white-space: nowrap;width: 90px;overflow: hidden;padding: 0 5px;font-size: 13px;}
    #path{background: white;border-bottom: 1px solid #CCC;padding: 5px;color: #333;}
    #explorercontainer{height:100%;min-width:465px;overflow: hidden;overflow-y: scroll;-webkit-box-flex: 1;-moz-box-flex: 1;box-flex: 1;}
</style>
<script>
    $(document).ready(function() {
	$("#explorer li.dir").live('click',function(){
	    var path = $(this).attr("path");
	    list(path);
	    $(this).next().find(" > li.dir").show();
	    $(this).next().slideToggle("fast");
	});
	$("#explorercontainer .dir").live('dblclick',function(){
	    var path = $(this).find(".explorer_file_name").attr("path");
	    list(path);
	});
	$(".explorer_file").live('click',function(){
	    $(".explorer_file_selected").removeClass("explorer_file_selected");
	    $(this).addClass("explorer_file_selected");
	    $("#<?php echo $_POST['idelmt']; ?>", parent.window.document).val("url(" + $(".explorer_file_name",this).attr("file").replace('/<?php echo PROFILE_PATH; ?>','<?php echo BASE_PATH; ?>') + ")")
	    parent.ParsimonyAdmin.trigger("#<?php echo $_POST['idelmt']; ?>",'keyup');
	});
...
<div style="background: #fff;">
    <div id="path">/</div>
    <div id="explorerWrap">
	<ul id="explorer" class="inline-block">
	    <?php

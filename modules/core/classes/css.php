...
class css{
	...
	while($len > $this->cursor){
	    if(!isset($this->selectors[$ids]['b'])) $this->selectors[$ids]['b'] = '';
	    switch ($find) {
		case 'next':
		    
		    /* Test comment */
		    $comment = strpos($css, '/*', $this->cursor);
		    $comment = $comment ? $comment : 999999999;
	...
	if(!isset($last['s'])) array_pop($this->selectors);
    }
	...
	$output = '';
	 ...
	 if(substr($media,0,1) == '@') $output .= PHP_EOL.'}';

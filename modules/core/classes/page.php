...
class page extends \block {
	...
	if (!empty($this->CSS_inc)){
            if(ID_ROLE != 1 || defined('PARSI_ADMIN')) $html .= PHP_EOL . "\t\t" . '<link rel="stylesheet" type="text/css" href="' . BASE_PATH . 'concat?format=css&files=' . implode(',', $this->CSS_inc) . '" />';
	    else{
        ...
        if (!empty($this->JS_inc)){
            if(ID_ROLE != 1 || defined('PARSI_ADMIN')){ 

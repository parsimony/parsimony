...
<?php if ($this->getConfig('mode') !== 'r') : ?>
	...
	<script>
		...
		$(myForm).on("change", "select", function() {
			...
			$.post(BASE_PATH + '<?php $mod = $_POST['typeProgress'] == 'theme' ? THEMEMODULE : MODULE; echo $mod; ?>/callBlock', {idPage: "<?php if ($_POST['typeProgress'] == 'page') echo $_POST['IDPage']; ?>", theme: "<?php if ($_POST['typeProgress'] == 'theme') echo THEME; ?>", id: "<?php echo $_POST['idBlock']; ?>", method: 'generateView', module: db[0],entity: db[1], update: document.getElementById('updateparam').value}, function(data) {

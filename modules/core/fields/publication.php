...
class publication extends \field {
	...
	public function addColumn($fieldBefore = '') {
		...
		return \PDOconnection::getDB()->exec('ALTER TABLE ' . $this->entity->getModule() . '_' . $this->entity->getName() . ' ADD ' . $this->name . ' DATETIME NOT NULL '.$pos.';
			ALTER TABLE ' . $this->entity->getModule() . '_' . $this->entity->getName() . ' ADD ' . $this->name . '_status INT(1) NOT NULL '.$pos.';
			ALTER TABLE ' . $this->entity->getModule() . '_' . $this->entity->getName() . ' ADD ' . $this->name . '_visibility VARCHAR(25) NOT NULL '.$pos);
	}

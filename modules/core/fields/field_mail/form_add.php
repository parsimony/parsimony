...
<script>
	...
	$(document).on("keyup", "#insert_<?php echo $this->name ?>", function(){
		...
		if(data == 1){
		    $(".info_insert_<?php echo $this->name ?>").empty();
		}else{
...
<div class="placeholder">
	...
	<input type="text" name="<?php echo $this->name ?>" id="insert_<?php echo $this->name ?>" class="<?php echo $this->name ?>" value="<?php echo $this->default ?>" <?php if(!empty($this->regex)) echo 'pattern="'.$this->regex.'"' ?> <?php if($this->required) echo 'required' ?> />
